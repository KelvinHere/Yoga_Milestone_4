...
class TestBasketViews(TestCase):
    ...
    def test_discount_met(self):
        ...
        test_grand_total = round(lesson.price - ((lesson.price / 100) * settings.DISCOUNT_PERCENTAGE), 2)
        # Do totals match
        ...
        self.assertContains(response, html.escape(f'Grand Total: €{grand_total}'))
        self.assertContains(response, f'{settings.DISCOUNT_PERCENTAGE}% discount applied!')
        
        # Discount banner threshold met
        self.assertContains(response, f'A {settings.DISCOUNT_PERCENTAGE}% discount has been applied to your basket!')
    ...
    def test_discount_not_met(self):
        ...
        self.assertContains(response, html.escape(f'Spend €{discount_delta} more for a {settings.DISCOUNT_PERCENTAGE}% discount!'))
        
        # Discount percentage
        self.assertContains(response, html.escape(f'Get a {settings.DISCOUNT_PERCENTAGE}% discount when you spend more than'))
